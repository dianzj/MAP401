...
Ensemble_Contours ajouter_element_ensemble_contours(Ensemble_Contours ensemble, Liste_Point L){
	ensemble.tab = realloc(ensemble.tab, (ensemble.nb + 1) * sizeof(Liste_Point));
	if (ensemble.tab == NULL)
	{
		fprintf(stderr, "ajouter_element_ensemble_contours : allocation impossible\n");
		exit(-1);
	}
	ensemble.tab[ensemble.nb] = L;
	ensemble.nb++;
	return ensemble;
}