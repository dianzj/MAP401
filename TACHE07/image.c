...
{
	if (x<1 || (unsigned int)x>I.la_largeur_de_l_image || y<1 || (unsigned int)y>I.la_hauteur_de_l_image)
		return BLANC;
...
void ecrire_image(Image I)
{
	...
	for (i=0; i<H; i++){
		for (j=0; j<L; j++){
			switch (get_pixel_image(I,j+1,i+1))
			{
		case BLANC:
			printf("⬜");
			break;
...
Image negatif_image(Image I)
{
	...
	for(i=0; i<H; i++){
		for(j=0; j<L; j++){
			...
			{
			case BLANC:
				set_pixel_image(I2,j+1,i+1,NOIR);
				
			break;
		case NOIR:
			set_pixel_image(I2,j+1,i+1,BLANC);
